...
const benchmarkSchema = z.object({
  userCV: z.any(), // User's CV data
  benchmarkCVs: z.array(z.any()).optional(), // Optional benchmark CVs
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validatedData = benchmarkSchema.parse(body);
    ...
    const { planType, ...benchmarkRequest } = validatedData;

    // Perform CV benchmarking
    const result = await premiumAIService.benchmarkCV(
      benchmarkRequest,
      session.user.id,
      planType
    );
    ...
    console.error('CV benchmarking API error:', error);
